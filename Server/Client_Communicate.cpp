        ...
        for(;;) {
            try {
                ...
                for(int i =0; i<count; i++) {
                    ...
                    std::unique_ptr<int16_t[]> vector_data(new int16_t[vector_len]);
                    rc = recv(work_sock,vector_data.get(),vector_len*sizeof(int16_t),0);
                    if(rc<=0) {
                        close(work_sock);
                        throw no_crit_err("[Uncritical error]vector not received");
                    }
                    std::vector<int16_t> v(vector_data.get(),vector_data.get()+vector_len);
